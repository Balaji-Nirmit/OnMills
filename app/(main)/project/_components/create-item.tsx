...
const CreateItem = ({ projectTitle, projectId, items }: CreateItemProps) => {
    ...
    const handleDelete = async () => {
        if (deleteId) {
            deleteItemFn(deleteId, projectId);
        }
        setDeleteOpen(false);
        setDeleteId(null);
        router.refresh();
        ...
        <>
                    ...
                    <div className="space-y-2">
                        ...
                        {items.map((item) => (
                            <Item key={item.id} className="group border-[#F2F0EB] hover:border-[#FF7A5C]/30 transition-colors">
                                ...
                                <ItemActions>
                                    {canChange && <Button disabled={deleteItemLoading} onClick={() => { setDeleteId(item.id); setDeleteOpen(true); }} variant="ghost" size="icon" className="h-8 w-8 text-[#86868B] hover:text-red-500 hover:bg-red-50 transition-colors">
                                        <Trash size={14} />
                                    </Button>}
                                </ItemActions>
                            </Item>
            ...
            <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
                <DialogContent>
                    ...
                    <p>Deleting this will DELETE ALL RELATED processing batches.</p>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => { setDeleteOpen(false); setDeleteId(null); }}>
                            Cancel
                        </Button>
                        <Button variant="destructive" onClick={handleDelete} disabled={deleteItemLoading}>
